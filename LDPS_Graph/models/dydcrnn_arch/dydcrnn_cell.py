...
class DCGRUCell(torch.nn.Module):
    ...
    def _build_sparse_matrix(L):
        ...
        L = torch.sparse_coo_tensor(indices.T/1.0, L.data, L.shape, device=device)
        return L

    def _get_supports(self, adj_mx):
        _supports = []
        supports = []
    ...
    def forward(self, inputs, hx, adj_mx):
        ...
        if self._use_gc_for_ru:
            ...
            fn = self._fc
        value = torch.sigmoid(fn(inputs, hx, output_size, adj_mx,bias_start=1.0))
        ...
        c = self._gconv(inputs, r * hx, self._num_units, adj_mx)
    ...
    def _gconv(self, inputs, state, output_size, adj_mx, bias_start=0.0):
        ...
        _supports = self._get_supports(adj_mx)
        ...
        # Adds for x itself.
        num_matrices = len(_supports) * self._max_diffusion_step + 1
        x = torch.reshape(
